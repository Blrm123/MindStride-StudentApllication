...
export default function TeacherDashboard() {
  ...
  const [phoneUsage, setPhoneUsage] = useState<PhoneUsageData | null>(null);
  ...
  // Fetch desktop activity summary and phone usage using same backend as student/parent dashboards
  async function fetchActivityDataForStudent() {
    try {
      ...
      console.error('Error fetching activity data for teacher view:', error);
      setActivityData({});
    }
  }

  async function fetchPhoneUsageForStudent() {
    try {
      ...
      console.error('Error fetching phone usage for teacher view:', error);
      setPhoneUsage(null);
    }
  }

  // Derived performance score for selected student using same logic as student/parent dashboards
  const selectedPerformanceScore = (() => {
    if (!selectedStudent) return null;

    const cgpaNum = parseFloat(selectedStudent.cgpa || "0") || 0;
    const attendance = typeof selectedStudent.attendance_percentage === 'number'
      ? selectedStudent.attendance_percentage
      : typeof selectedStudent.attendance === 'number'
        ? selectedStudent.attendance
        : 0;

    const desktopStudyHours = (activityData.study || 0) / 60;
    const desktopEntertainmentHours = (activityData.entertainment || 0) / 60;
    const phoneHours = phoneUsage ? phoneUsage.total_minutes / 60 : 0;
  ...
  const handleStudentClick = async (student: Student) => {
    ...
    // Fetch screen time data for visualization (mock) and real usage data for performance score
    const mockScreenTime: ScreenTimeData = {
      phone_usage: Math.floor(Math.random() * 480),
      desktop_usage: Math.floor(Math.random() * 360),
      total_usage: 0,
    };
    mockScreenTime.total_usage = mockScreenTime.phone_usage + mockScreenTime.desktop_usage;
    setScreenTimeData(mockScreenTime);

    // Real usage for performance score (same backend as student/parent dashboards)
    fetchActivityDataForStudent();
    fetchPhoneUsageForStudent();
  };
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-lg font-semibold">
                          {selectedPerformanceScore !== null ? selectedPerformanceScore : 'N/A'}
                        </p>
                      </div>
                    </div>
                  </CardContent>
